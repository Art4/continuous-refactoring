...
**Every write to the target is reported on its own, as it happens** — an issue created, a comment or label on an issue, a branch pushed, a merge request opened or marked ready, a bookkeeping merge request, an `out-of-scope/` entry. Name the thing and where it lives (number, branch, link). Say "merge request" or the forge's own word (pull request on GitHub), per `opening-a-merge-request.md`.
