...
**Pending issue:** none
**Fulfilled nodes:** loop-config, composer, ci-runner
```
...
| `Fulfilled nodes` | Tooling-tree node **slugs** (never Names — internal bookkeeping stays keyed by the slug) already confirmed fulfilled, comma-separated | `refactor-learn`, every closing call — see *Fulfilled nodes* below |
...
- **`Pending issue` and `Fulfilled nodes` are `refactor-learn`-written — never by hand.** `Create-mode` and `Focus areas` you can edit by hand any time — that's what they're for. Nobody is expected to hand-edit `Fulfilled nodes`; if it drifts wrong, the next pass with parser access overwrites it.
- The file travels with the repo. Loop state does not live in agent sessions but here (create-mode, focus areas, pending issue, fulfilled nodes), in the issue tracker (backlog), in `docs/refactoring/merge-requests.md` (open suite merge requests — only when the target's issue tracker has no native label mechanism; otherwise that state lives directly on the tracker as `refactor:delivered`-labeled issues), and in `docs/refactoring/out-of-scope/` (learned rejections).
