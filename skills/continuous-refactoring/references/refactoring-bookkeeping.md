...
`Fulfilled nodes` sorts last, not alphabetically or by write-frequency — it's the field most likely to grow long as the tree gets worked through, and the only-ever-growing one; keeping it below every other field means `Create-mode`, `Focus areas`, `Refactoring goal`, `Housekeeping cadence`, and `Pending candidates` stay visible together without scrolling past it.
...
| `Fulfilled nodes` | Tooling-tree node **slugs** (never Names — internal bookkeeping stays keyed by the slug) already confirmed fulfilled, one per bulleted line, each carrying the delivering issue # (`- <slug> (#<issue>)`) when known — see *Fulfilled nodes* below. Sorts last — see the note above the table. | `refactor-learn`, closing call, only when that call has a genuine delivery or rejection to record this pass (`docs/adr/0051-refactor-learn-requires-a-genuine-event.md`) — never written by itself as the sole reason for a bookkeeping branch — see *Fulfilled nodes* below |
...
- **Write:** `refactor-learn`, closing call, per its own `## Process` — but only when that call has a
  genuine delivery or rejection to record this pass; never opened as a branch's sole reason
  (`docs/adr/0051-refactor-learn-requires-a-genuine-event.md`). When it does write and this pass ran the
