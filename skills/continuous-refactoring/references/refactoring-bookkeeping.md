...
- If the file is missing, the target isn't onboarded yet: the dispatcher's onboarding step runs before anything else, and every other skill that needs the file aborts with a pointer to `/continuous-refactoring` rather than creating it.
