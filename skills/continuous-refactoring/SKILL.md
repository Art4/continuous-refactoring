...
1. **Scan.** Run `/refactor-scan` — checks preconditions (git, backlog size) and resumes `Pending candidates` before proposing anything fresh. Also decides, internally, whether the **Safety Net Track** (`CONTEXT.md`) is due this pass and resumes its own `Open` list the same way (`skills/refactor-scan/references/safety-net-track.md`) — the orchestrator itself makes no Track-selection decision yet; only this one Track is wired up so far, more follow later.
