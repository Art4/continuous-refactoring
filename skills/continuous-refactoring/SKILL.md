...
- **Config** — `docs/refactoring/config.md`: focus areas, merge-request create-mode, the `Pending tasks` marker, and the `Fulfilled nodes` cache (`skills/continuous-refactoring/references/refactoring-config.md`)
...
1. **Scan.** Run `/refactor-scan`. It checks its own preconditions first — git, backlog size — and resumes any `Pending tasks` before proposing anything fresh. Two outcomes end the pass here, before anything else runs: no git repository (nothing else runs, not even step 5), or the backlog is already full (skip to step 5 with whatever findings scan reported, no new candidate this pass). A third outcome — scan detects an already-open merge request with newer reviewer activity than its own last commit — is a **resume-candidate**: skip straight to step 5 with it (steps 2–4 don't run; the candidate already has a design and an open merge request, only the fix loop applies). Every other outcome carries scan's **findings** (closed/merged issues or MRs, possibly empty) into step 2 and **proposals** (the resumed pending task, or up to five node Names, never slugs) into step 3.
