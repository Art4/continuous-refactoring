...
For a tooling-tree candidate, close with an outlook: re-run `python3 skills/refactor-scan/references/tooling_tree.py <target-repo> --steps 1` against the now-changed working tree, look up whatever node slug it reports next in the tree doc, and name it by its **Name** (never the slug the script returned), working its Purpose into that same sentence in plain language — one flowing sentence a human can read, not a `Purpose:`-labelled field and not a citation of the tree doc. If `python3` isn't available or running it isn't permitted, dispatch a sub-agent with `skills/refactor-scan/references/tree-walk-prompt.md`'s prompt (`{N}=1`) instead; with no sub-agent mechanism, run that prompt's steps yourself inline. A structural candidate carries no outlook — there's no single "next child" a deepening unlocks the way a tree node does. No type enum — outlook is settled, a type enum is a separate, still-undecided question.
