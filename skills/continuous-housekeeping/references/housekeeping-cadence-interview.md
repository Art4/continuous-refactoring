...
Recommendation: **7 days**. Nothing about this interview blocks on anything else — ask it standalone, a single `AskUserQuestion` call when available, the same numbered-prose shape otherwise (`skills/refactor-design/references/grilling-fallback.md`'s convention).
