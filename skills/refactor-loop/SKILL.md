...
State lives in the target repo, not the conversation. Every lifecycle skill reads it directly; each writes only the field its own step produces (`refactor-design` sets `Pending candidates` for the candidate it plans — for a gate-shaped one on every tracker, for a tooling-tree node or an externally-labeled one on a non-native tracker only) — `refactor-learn` writes everything else, and is the suite's only *dedicated* bookkeeping writer:

- **Config** — the Refactoring Notes' `bookkeeping.md`: focus areas, `Ticket-create-mode` and `MR-create-mode` (decided once, during the dispatcher's onboarding step — see `## Creating a ticket` and `## Opening a merge request`), `Pending candidates`, and every currently-wired **Track**'s (`CONTEXT.md`) own bookkeeping section — `## Safety Net`, `## Guardrails`, `## Housekeeping`, `## Investigation` (`../continuous-refactoring/references/refactoring-bookkeeping.md`).
...
The Refactoring Notes' `bookkeeping.md` isn't scaffolded here — `continuous-refactoring`'s onboarding step writes it (the `onboarding-setup` node, fulfilled before any Track runs), with `Ticket-create-mode` and `MR-create-mode` already set from its interview. This skill only checks that it exists (`## Input`).
