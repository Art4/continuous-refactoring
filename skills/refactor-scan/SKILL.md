...
Read the Refactoring Notes' `bookkeeping.md`'s `Pending candidates` field (`skills/continuous-refactoring/references/refactoring-bookkeeping.md`), **and** the `Open` list of whichever Track the orchestrator's own Track-selection step selected this pass (`skills/continuous-refactoring/references/track-scheduler.md`) — a Safety Net or Guardrails Track candidate resumes the same way, just tracked separately (`skills/refactor-scan/references/safety-net-track.md`, `skills/refactor-scan/references/guardrails-track.md`). Only the selected Track's own `Open` list is ever relevant here in practice: a Guardrails node is never unblocked until the Safety Net has fully resolved, so the two Tracks' `Open` lists can't both be non-empty at once anyway (`guardrails-track.md`'s own Scope section) — `track-scheduler.md`'s fixed tie-break order exists for when Housekeeping/Investigation, which carry no such gating relationship, join and this stops being structurally impossible. If any names an issue, a prior pass got partway through this candidate before being interrupted — finishing pending work comes before proposing fresh work. Read the issue for a plan (`refactor-design`'s output — a comment for most candidate types, the issue body itself for a tooling-tree node/`loop-config`) to see how far it got:
