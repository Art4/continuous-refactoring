...
- No Refactoring Notes' `bookkeeping.md` yet (`loop-config` unfulfilled) → propose `loop-config` alone, skip straight to step 4's `loop-config` output. Steps 2/3/3b all read state that only exists once `loop-config`'s own interview has scaffolded it (`Pending candidates`, the remembered set, `docs/agents/issue-tracker.md`) — nothing there yet to read, and no other node can be unblocked either (every one requires `loop-config` as a parent). No `tooling_tree.py` run, no tree-doc reading needed to reach this: a target's very first pass costs one conclusion, not a walk of the whole tree.
