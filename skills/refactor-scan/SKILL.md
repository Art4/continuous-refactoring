...
- No Refactoring Notes' `bookkeeping.md` yet (`onboarding-setup` unfulfilled) → skip steps 2 through 4c entirely and hand forward `onboarding-setup` as the pass's only proposal (the shape step 4's Output section already describes). No `tooling_tree.py` run, no tree-doc reading needed to reach this: steps 2/3/3b all read state that only exists once `onboarding-setup`'s own interview has scaffolded it (`Pending candidates`, the remembered set, `docs/agents/issue-tracker.md`), and no other node can be unblocked either (every one requires `onboarding-setup` as a parent) — a target's very first pass costs one conclusion, not a walk of the whole tree.
...
for a tooling-tree node/`onboarding-setup` — the issue body itself), not a proposal to rank.
...
- **`psr-4` exception — the entry-point walk over-reports; judge each file it flags.** Judging `psr-4`'s Fulfilment check (its criterion-2 entry-point walk, `php-tooling-tree/psr-4.md`'s own *"Reading a non-empty `unwired_entry_points` result"* section) yields a deliberately blunt, over-reporting-prone raw fact — every candidate file the walk flags as unwired — not a verified conclusion; the judgement call is this skill's job, never the script's (`detected`'s `details` is always empty now — fulfilment comes from the seed). Before accepting `psr-4` as still open (in `next`, or blocking a downstream node from `next`/`withheld`) because of a flagged file, look at each one: a file that never references the target's own PSR-4 root namespace and is self-evidently a dev/CI/build utility is exempt — skip it. If every flagged file is exempt this way, treat `psr-4` as fulfilled after all; otherwise the remaining, non-exempt files are real unwired work, proposed as usual.

- **Ordinary tooling nodes** (`onboarding-setup`, and language-specialization nodes, e.g. `skills/refactor-scan/references/php-tooling-tree.md`) — proposed by their **Name** (never the raw slug); each is already fully specified in its tree doc. Already forwarded this pass via step 3b (a prior pass's own pre-filing, `refactor-prioritize/SKILL.md` step 2, already turned it into an issue) → don't propose it again by bare Name here, it's already in the list as that issue.
...
the `detected` map from step 4's own `tooling_tree.py` run (no second invocation — its `fulfilled`
values come from the seed/bookkeeping, and its `details` is always empty) — find the highest `N`
where `phpstan-level-N` is `fulfilled`. Check that level's baseline yourself, per `phpstan.md`'s
...
(`skills/refactor-scan/references/tooling-tree.md`'s own `secret-detection` node). Read the
`detected` map from step 4's own `tooling_tree.py` run (no second invocation — its `fulfilled`
values come from the seed/bookkeeping, and its `details` is always empty) — `secret-detection`'s own
`fulfilled` is `true`, and the Refactoring Notes' `bookkeeping.md`'s `Secret history scan` field is
...
check names the recognized scanners; the script's output no longer carries a scanner name) — against
