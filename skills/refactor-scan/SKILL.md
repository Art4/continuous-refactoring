...
- **`structural-scan`** — proposed once every node with a `resolved` edge into it is resolved (fulfilled, or explicitly rejected under `docs/refactoring/out-of-scope/`) — almost all of them the active language tree's own leaves, plus `editorconfig` at the generic root (`skills/refactor-scan/references/tooling-tree.md`). Proposing it is just naming it (by its Name) — no codebase walk happens here (that's `refactor-design`'s job, only for the node actually chosen).
