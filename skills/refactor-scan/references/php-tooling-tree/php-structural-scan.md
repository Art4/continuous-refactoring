...
- **Fulfilment check:** every one of its eleven `resolved` parents (`psr-4` — `psr-4.md`; `composer-audit` — `composer-audit.md`;
  ...
  reasoning); `phpmd` and the generic root's `secret-detection` are Signal-producing nodes adjacent to this
  tree that deliberately never became leaves here at all.
