...
- **`php-safety-net`'s resolved-gate** gains this node as one of its nine leaves (`php-tooling-tree.md`'s edge table) — structural work waits until it's *decided* (fulfilled at the bar above, or explicitly rejected), the same way every other convention/check in that leaf set already gates structural work, on the reasoning that agent-driven structural changes are safer once the codebase's own organizational conventions are settled.
