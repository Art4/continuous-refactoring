...
**`refactor:priority` narrows the pool first.** At least one surviving proposal's issue already carries the `refactor:priority` label (a human set it directly — nothing here sets it automatically) → rank only among those this pass, the rest wait for a future one regardless of how they'd otherwise score. None carry it → rank the full surviving pool, unchanged.
