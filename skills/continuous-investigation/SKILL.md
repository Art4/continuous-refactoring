...
The **Investigation** Track (`CONTEXT.md`) as one loop pass. Runs the Track's single `structural-scan` gate — the scheduler's permanent fallback, always due — and files the concrete candidate it selects. That Track's own logic lives in `../refactor-scan/references/investigation-track.md` and `../refactor-learn/references/investigation-write.md`, unchanged. This skill only names the Track and hands it to `refactor-loop`, which runs the pass.
