...
The **Guardrails** Track (`CONTEXT.md`) as one loop pass. Works through the Guardrails nodes of the tooling tree — the ones proposed only once the Safety Net has closed — walking the Track's own `Open` list or scanning afresh. That Track's own logic lives in `../refactor-scan/references/guardrails-track.md` and `../refactor-learn/references/guardrails-write.md`, unchanged. This skill only names the Track and hands it to `refactor-loop`, which runs the pass.
