...
**`loop-config` exception — this node itself:** `refactor-design` couldn't write `Pending candidates` (`docs/refactoring/config.md` didn't exist yet). When you create the file here, set `Pending candidates` to this candidate's issue, and set `Create-mode` (and `Focus areas`, only if the candidate issue's recorded decisions name one) from the plan `refactor-design` filed — the interview already decided these, don't leave them unset or invent placeholders. Leave `Fulfilled nodes` out (or clearly unset) — per `skills/continuous-refactoring/references/refactoring-config.md` that field is still `refactor-learn`'s, filled in its own follow-up commit. If the candidate issue's recorded decisions named a local tracker, also create `docs/agents/issue-tracker.md` here, from `skills/continuous-refactoring/references/local-issue-tracker-template.md` — same merge request as `config.md`, this node's only MR.
