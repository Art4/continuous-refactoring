...
Every slice follows the foundational refactoring rules: `../continuous-refactoring/references/foundational-refactoring-rules.md`.
...
- **`/tdd`**: installed → use its discipline. Otherwise loop rules and test-quality criteria inlined at `references/review-fallback.md`.
- **`mattpocock/skills` implement skill**: installed → it already embeds step 4's review. Otherwise run step 4's two axes by the standards-axis rules (repo standards plus a fixed Fowler smell set) at the same `references/review-fallback.md`.
