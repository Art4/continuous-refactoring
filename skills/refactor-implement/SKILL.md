...
**`loop-config` exception — this node itself:** `refactor-design` couldn't write `Pending candidates` (the Refactoring Notes' `config.md` didn't exist yet). Creating the file here, set `Pending candidates` to this candidate's issue, and `Create-mode` (and `Focus areas`, only if named) from the plan `refactor-design` filed — the interview already decided these, don't leave them unset or invent placeholders. Leave `Fulfilled nodes` unset — still `refactor-learn`'s field, filled in its own follow-up commit. Interview named a local tracker → also create `docs/agents/issue-tracker.md` from `skills/continuous-refactoring/references/local-issue-tracker-template.md`. Also write the `Refactoring Notes:` line into whichever of `AGENTS.md`/`CLAUDE.md` the interview's Explore step found (never create the other instead), per `skills/continuous-refactoring/references/loop-config-interview.md`'s `## Record`. All of it in the same MR as `config.md` — this node's only one.
