...
- Clear `docs/refactoring/config.md`'s `Pending tasks` field — this candidate now has a merge request, so the marker that would let `refactor-scan` resume it as unfinished work no longer applies.
