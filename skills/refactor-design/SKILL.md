...
**`onboarding-setup` exception:** chosen node *is* `onboarding-setup` → the issue body isn't the tree doc's generic spec, it's what `skills/continuous-refactoring/references/onboarding-setup-interview.md`'s `## Record` names (the interview's decisions, each with its one-line rationale). And `bookkeeping.md` doesn't exist yet, so no `Pending candidates` to write here either — `refactor-implement` records it directly when it creates the file.
...
pass — for a tooling-tree node or `onboarding-setup`, the gate never runs at all, so this step is the only
...
fully-designed issue on sight, whether its plan lives in the body (tooling-tree node, `onboarding-setup`)
