...
**A PHPStan baseline-shrink candidate** — same shape: `refactor-prioritize`'s Select mode already picked the group and filed it minimally (`refactor-scan` step 4b's proposal, refined). Run `references/phpstan-baseline-shrink.md` step 3 (plan the fix), then continue here at step 5. Steps 3–4 above (the structural-candidate reference) don't apply — this candidate's own reference file is the whole procedure.
