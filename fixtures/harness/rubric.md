...
| 4 | **State hygiene** | `.scratch/refactor/bookkeeping.md` / `merge-requests.md` left inconsistent with what actually happened (stale `Pending candidates`, missing `Open` entry) | State exactly reflects the pass: `refactor-learn` wrote it once, last, and it matches git history |
