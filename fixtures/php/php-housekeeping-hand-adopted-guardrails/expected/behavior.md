...
gets its `Housekeeping` checklist line via agent judgement in the reconciliation step.
...
7. The reconciliation uses agent judgement exclusively.
...
delivering MR, if any).

## The behavior this regression-tests

A reconciliation that only reacted to delivering MRs would never see a hand-adopted Guardrails
tool, so its housekeeping line would be permanently missing. Agent judgement closes this gap.
