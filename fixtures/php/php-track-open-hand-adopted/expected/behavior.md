...
(`skills/continuous-refactoring/references/track-open-processing.md`) re-runs the Fulfilment check
before creating a node's issue, and removes a node that has become fulfilled since the scan (for
example adopted by hand) without creating a merge request.

Not deterministically checkable via `tooling_tree.py` — the parser has no notion of Tracks or the
`Open` walk; this is a behavioral property of `continuous-refactoring/SKILL.md` step 1's Track `Open`
handling, checked the same non-CI, local-only, advisory way `safety-net-track`/`guardrails-track`
already are. Run via `fixtures/harness/run.sh safety-net-track php-track-open-hand-adopted
--opencode`.
...
2. Walk the list top to bottom per `track-open-processing.md`:
   - **php-cs-fixer (#5)** — workable (unblocked, no `needs-info`, no PHP floor issue). Re-run its
     ...
     committed, the tool runs clean. **Fulfilment check now passes** → remove `php-cs-fixer` from
     `Open` with no merge request, no issue created. Move to the next entry.
...
4. `php-cs-fixer` must **not** have a merge request opened for it — it left `Open` silently.
...
removes the node from `Open` silently.
