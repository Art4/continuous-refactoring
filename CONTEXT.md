...
**Fulfilment check**:
A node's own test for whether it's already adopted — evaluated against the node's own Purpose
statement by an agent walking the tree during its **Track**'s (above) scan, recognizing any real,
working tool that serves that Purpose rather than matching a fixed list of names (a
Laravel-Pint-configured target satisfies `php-cs-fixer`'s Fulfilment check the same as a
directly-configured one). Lives in the node's own tree-doc entry, alongside its **MR scope** (what an
adoption's merge request actually delivers) — every node carries both, together they're what "a node"
(above) means operationally.
