...
- [ ] A `refactor:priority`-labeled issue narrows the Rank pool and is otherwise only a tie-breaker between equally ranked candidates; it never preempts a Track's `Open` walk, does not bypass the selected Track or the Safety Net blockade, and waits until `Open` is empty (Safety Net) or has no workable node left (Guardrails).
