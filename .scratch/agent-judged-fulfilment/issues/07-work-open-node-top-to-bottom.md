...
- [ ] A `refactor:priority`-labeled issue is preferred over the top of `Open`, and does not bypass the selected Track or the Safety Net blockade.
- [ ] Skipped non-workable nodes appear with a reason in the pass report.
- [ ] Advisory agent fixtures cover: pick-up re-check on a hand-adopted node, top-to-bottom order with a blocked node in between, and a priority-labeled issue versus the top of `Open`.
