...
- **Migration order.** Audit and gate nodes first (no runtime change), then `Open` semantics and scheduler documentation in parallel with the script reshaping, then the skills' switch to agent input, then the tests, and last the deletion of detection code. Until the last step each ticket is independently revertible.
...
- **Ticket slicing** (for `/to-tickets`, blocked-by order): (1) prose audit of every parser heuristic against its tree doc, plus the three gate nodes with edges and prose checks; (2) `Open` semantics, scheduler documentation and bookkeeping schema, with the new ADR; (3) script reshaped to graph-only with the new inputs and outputs, roadmap removed, detection still present; (4) skills switched to agent input, no pre-filing for Track nodes, pick-up re-check, `Open` maintenance in `refactor-learn`, `Fulfilled nodes` read and write sites removed, Housekeeping sweep over the five nodes; (5) tests and the drift check; (6) detection code and its tests deleted. Tickets 2 and 3 can run in parallel after 1.
- **ADR.** One new ADR replaces the earlier decision to ship the parser with detection under `refactor-scan`, amends the pre-filing decision for Track nodes, and amends ADR-0055 on the meaning of `Open` and on `Fulfilled nodes`.
- **Existing tickets 08 and 09** (open pull requests) are absorbed here: 08's behavioral premise no longer holds, only its documentation correction remains and is part of ticket 2; 09 is covered by tickets 1 and 4. Those pull requests are closed once this spec's tickets exist.
- **Glossary.** The implementing tickets update the **Track** entry (Safety Net blockade, Guardrails yielding) and the **Proposals** entry (no pre-filing for Track nodes), and add an entry for the recognition-only gate node.
- **Known detail left to tickets.** The fulfilled-set file format, and whether the stalled report also appears as an issue comment or only in the pass status.
