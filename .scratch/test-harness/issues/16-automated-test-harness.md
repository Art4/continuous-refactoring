...
> **2026-08-20:** Implemented. `scripts/validate_skills.py` (Python stdlib + pyyaml, no LLM) with `scripts/test_validate_skills.py` (unittest, TDD at the function seams + one end-to-end test against the real repo) and a GitHub Actions gate `.github/workflows/skills-validation.yml` (runs tests, then the validator on push/PR touching `skills/`, `docs/`, `CONTEXT.md`, `scripts/`). Checks: frontmatter (name = directory, kebab-case, description, known fields incl. opencode extensions), required sections (`## Completion criterion` everywhere, `## Process` on lifecycle skills / `## The pass` on the orchestrator, `## Fallback` on ADR-0003-shipped skills per the ledger), bidirectional `/X`↔ledger consistency (backticked or bare), local `docs/`/`CONTEXT.md`/`*.md` refs resolve (target-repo artifacts exempt, whitespace-bearing backticks ignored), `ADR-NNNN` resolves, glossary terms in use + avoid-term scan (plural-aware) with documented allowlist entries. Required rewording 3 avoid-term uses in skills (session→conversation, grilling session→grilling); `boundary` in refactor-design and `pain point` in refactor-scan kept via allowlist — both are legitimate prose, not synonym replacements. Errors only, exit codes 0/1.
