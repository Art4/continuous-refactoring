...
- [x] `scripts/test_tooling_tree.py`: `RecommendedGateTests` (7 new tests) covering withhold/release/fulfilled/rejected/cascade/multi-parent/uncapped behavior. Full suite (`python3 -m unittest discover -s scripts -p 'test_*.py'`, 114 tests) and `python3 scripts/validate_skills.py .` both pass clean — no `ADR-####` citations leaked into any shipped skill file or reference doc.
