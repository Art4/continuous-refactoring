...
>
> Amended by [ADR-0012](0012-remembered-merge-requests-follow-the-tracker.md): `refactor-learn` is no longer the *unconditional* writer of `docs/refactoring/merge-requests.md` — when the target's issue tracker natively supports labels, that state lives on the tracker instead and nothing is written to a file.
...
Also surfaced, and fixed directly in the affected skill: a `loop-config` bootstrap gap (`refactor-design` can't set `Pending tasks` in a `config.md` that doesn't exist yet, since `loop-config` is the candidate that creates it) — see the `loop-config` exception in `refactor-design`, `refactor-implement`, and `refactor-learn`.
