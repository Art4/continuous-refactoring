...
Also surfaced, and fixed directly in the affected skill: a `loop-config` bootstrap gap (`refactor-design` can't set `Pending candidates` in a `config.md` that doesn't exist yet, since `loop-config` is the candidate that creates it) — see the `loop-config` exception in `refactor-design`, `refactor-implement`, and `refactor-learn`.
