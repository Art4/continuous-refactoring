...
Fallbacks are written inline in the skill file itself (no shared suite file — each skill must work standalone). Before invoking `/X`, check it is available in the environment; if not, follow the fallback. The complete reference inventory lives in `docs/agents/skill-references.md` — add or drop a row there in the same change that adds or drops a global reference in a skill.
