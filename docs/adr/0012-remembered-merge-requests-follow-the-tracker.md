...
`docs/refactoring/config.md` (`Last run`, `Create-mode`, `Focus areas`, `Pending issue`), ADRs, and `CONTEXT.md` are unaffected by either mode — none of that data is tracker-representable, and it remains committed, governed by ADR-0011, regardless of which mode applies to `merge-requests.md`.
