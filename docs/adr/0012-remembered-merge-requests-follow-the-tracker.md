...
`docs/refactoring/config.md` (`Create-mode`, `Focus areas`, `Pending tasks`, `Fulfilled nodes`), ADRs, and `CONTEXT.md` are unaffected by either mode — none of that data is tracker-representable, and it remains committed, governed by ADR-0011, regardless of which mode applies to `merge-requests.md`.
