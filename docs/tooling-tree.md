...
- **MR scope:** never an MR by itself — fulfilling this node just opens the gate. The actual work is `refactor-scan`'s ordinary codebase walk (hot spots, module/interface/depth/seam vocabulary), filing structural candidates the usual way once the gate is open.
