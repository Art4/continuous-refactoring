...
2. **For every new/updated open MR:**
   - **Draft** (`gh pr view <n> --json isDraft`) → not ready for review yet, skip it this round — no
     `request-changes`, no escalation, not a hang. The suite itself no longer opens candidate MRs as drafts,
     so a draft is someone's own work in progress. Only worth a second look if it's *still* draft across
     several consecutive rounds with no new commits — log it as a quiet observation, not an anomaly.
