...
|---|---|---|
| `Last run` | Date of the last completed pass | `refactor-learn` after each pass (ADR-0010) |
...
| `Pending issue` | The issue `refactor-design` just filed, not yet delivered as a merge request — `none` the rest of the time (ADR-0010) | `refactor-design` sets it when it files; `refactor-learn` clears it once the merge request is remembered (`merge-requests.md`) or the candidate is resolved another way |
