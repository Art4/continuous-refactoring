...
- **Config (PHPStan path):**
  ...
  - Dependency versioning follows the target's pinning policy; if none is declared, use a `^` caret range with a committed `composer.lock` (ADR-0005).
...
- **Stop conditions / when not to raise:**
  ...
  - CI is irrelevant to this node's gating: the fulfilment check is local (`vendor/bin/phpstan analyse` green). The CI-job child with two parents (tool + `ci-runner`) remains deferred per ADR-0007 and does not gate the level chain.
...
- **Psalm (`vimeo/psalm`) fulfils `phpstan-level-0-baseline`** per ADR-0005's "fill gaps, never downgrade; an equivalent fulfils the node" rule (same as Pest fulfils `phpunit`). Detection: `composer.json` lists `vimeo/psalm` and a Psalm config file (`psalm.xml` or `psalm.xml.dist`) exists. When present and green, the scan marks `phpstan-level-0-baseline` as fulfilled and does not propose the PHPStan introduce candidate.
