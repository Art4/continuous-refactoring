...
|---|---|---|---|---|
| `refactor-prioritize` | `/codebase-design` | enrichment (vocab already inline) | crash-safe | scan-design-split ticket 1 |
...
Exempt (no global refs): `refactor-scan`. `refactor-design`'s own `/codebase-design` reference moved to `refactor-prioritize` (scan-design-split ticket 1) — candidate search for a `structural-scan` winner is `refactor-prioritize`'s Select mode's job now, not `refactor-design`'s.
